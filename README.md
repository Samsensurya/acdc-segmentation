...
├── acdc_baseline_model.py   # Baseline single-stage 2D U-Net (DiceCE + InstanceNorm)
│                            # Used as a reference before building the cascaded pipeline
...
In `acdc_baseline_model.py`, set:
```python
ACDC_ROOT = Path("/your/path/to/ACDC")
```
Then run:
```bash
python acdc_baseline_model.py
```
