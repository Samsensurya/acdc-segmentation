...
- Schedule: **OneCycleLR** (87 epochs) → **SWA** at lr=5e-5
...
```
├── ACDC_DiceCE_INSTANCE_AUG.ipynb  # Exploratory notebook: preprocessing, training
│                                   # experiments, and result comparisons
├── acdc_training.py                # Full training pipeline (Stage 1 + Stage 2)
├── acdc_inference.py               # Inference script for test set
├── acdc_eval.py                    # Evaluation: DSC, HD, ASSD, clinical metrics
└── README.md
```

> **Note:** `database/` (patient data) and `runs/` (model checkpoints) are excluded
> from this repository. See the Data section below.
...
[official challenge page](https://acdc.creatis.insa-lyon.fr) and place it
under `database/`:
...
> **Note:** Check the argument names inside each script before running —
> the commands below reflect the expected behaviour but exact flags may differ.
...
1. Baumgartner et al. *An exploration of 2D and 3D deep learning techniques for cardiac MR image segmentation.* STACOM, 2017.
1. Oktay et al. *Attention U-Net: learning where to look for the pancreas.* MIDL, 2018.
1. Karimi & Salcudean. *Reducing the Hausdorff distance in medical image segmentation.* IEEE TMI, 2020.
